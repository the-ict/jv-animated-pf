...
const AboutBackgroundContent = () => (
  <div className="flex flex-col pt-[40px] md:flex-row gap-10 md:gap-20 max-w-6xl w-full text-gray-100 overflow-y-auto max-h-[80vh] px-4 md:px-0">
    {/* Left side */}
    <div className="w-full md:w-2/3 leading-relaxed space-y-6">
      ...
      <p className="text-sm md:text-base">
        I worked in{" "}
        <span className="font-semibold text-blue-200">CAD modeling</span> for 2 years where I learned
        to collaborate with teams and work effectively under deadlines.
      </p>

      <p className="text-sm md:text-base">
        However, I realized that wasn’t the creative path I truly wanted to
        pursue. Outside of work, I began learning{" "}
        <span className="font-semibold text-blue-200">3D modeling and animation</span>, spending
        countless hours improving my artistic and technical skills.
      </p>

      <p className="text-sm md:text-base">
        Over time, my knowledge of 3D modeling grew significantly, and I
        developed a passion for{" "}
        <span className="font-semibold text-blue-200">character creation</span> — from sculpting to
      ...
      <p className="text-sm md:text-base">
        ...
        combining technical precision with creativity to bring characters to life.
      </p>
    </div>
    ...
    <div className="w-full md:w-1/3 space-y-4 text-sm text-gray-300">
      ...
      <div>
        <p className="font-semibold text-blue-200 text-sm md:text-base">3D CAD Modeler</p>
...
export default function AboutNavigationLink({ children, logoRef }: Props) {
  ...
  return (
    <>
      <div
        onClick={handleClick}
        className="text-sm md:text-base cursor-pointer text-gray-200 font-semibold hover:text-blue-50 transition-colors"
      >
        {children}
      </div>
      ...
      {showAboutContent && (
        <div
          ref={bgRef}
          className="fixed top-0 left-0 w-full h-full flex flex-col md:flex-row items-center justify-center px-6 md:px-[100px] z-20 overflow-y-auto"
        >
